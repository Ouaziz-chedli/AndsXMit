...
os.environ["DB_PATH"] = "/tmp/prenatal_test.db"
os.environ["DATA_DIR"] = "/tmp/prenatal_test"
os.environ["IMAGE_DIR"] = "/tmp/prenatal_test/images"

# Create test engine using file-based SQLite (in-memory creates new DB per connection)
TEST_DATABASE_URL = "sqlite:////tmp/prenatal_test.db"
...
def db_session():
    ...
    # Seed default diseases - use same engine as test_client
...
def client(db_session):
    ...
    # Override the get_db dependency - pass the db_session from fixture
...
def test_diagnose_mock_response(client):
    ...
    # Should return 3 diseases (down, edwards, patau in MOCK_RESULTS)
    assert len(result["fast_track"]) == 3
